...
describe("The calculator", () => {
  ...
  describe("does more complicated math", () => {
      ...
      it("Can multiply more than two numbers together:", async () => {
        ...
        await clickButton(driver, "5");
        await clickButton(driver, "=");
        let result1 = await getDisplay(driver);
        await clickButton(driver, "×");
        await clickButton(driver, "1");
        await clickButton(driver, "5");
        await clickButton(driver, "=");
        let result2 = await getDisplay(driver);
        expect(3.14159*15*15).toEqual(706.85775);
      });
      it("Can multiply by negative value:", async () => {
        await clickButton(driver, "2");
        await clickButton(driver, "2");
        await clickButton(driver, "+/-");
        await clickButton(driver, "-");
        ...
        await clickButton(driver, "=");
        let result = await getDisplay(driver);
        expect((-22-5+12)*200).toEqual(-3000);
      });
});
